...
class PlayListLoader {
  ...
  async readGoogleSheetPlaylist(gs_id, {title,video='n'}) {
    ...
    // });
    await doc.useServiceAccountAuth(require('./gdrive-auth.json'));
    await doc.loadInfo(); // loads document properties and worksheets
    const sheet = doc.sheetsByTitle[title];
    console.log('Loaded doc: '+doc.title+' by '+ doc.author);
    console.log(`sheet: ${sheet.title} ${sheet.rowCount}x${sheet.columnCount}`);
    // read rows
    ...
    const result = rows.reduce((o, {Name:name, VideoID:videoid}) => {
